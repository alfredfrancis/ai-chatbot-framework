...
def getIfsc(entities):
    ...
    if len(result) == 0:
        ...
        result = "<ol><li>"+result[0] + "</li><li>" + result[1]+"</li></ol>"
    return result
...
def getDetailsFromMobileNumber(entities):
    ...
    try:
        responseDict = json.loads(response.text)
        return "Name : %s <br> User ID : %s <br>DOB : %s <Br>Status : %s<br>Tell <i>Activate {user id} </i> for user Activation" % (
        responseDict.get("firstName") + " "
...
def checkH2HTransactionStatus(entities):
    ...
    2016-08-18 10:43:56.0

    05-02-2016 04:42:30 PM
...
def checkTransactionStatus(entities):
    ...
    if not (statusDesc):
        ...
        # entities["serviceProviderCode"] = responseDict.get('beneficiary').get("beneficiaryBank").get(
        #     "serviceProviderCode")
        # entities["uploadDate"] = responseDict.get('payinDate')
        # result = checkH2HTransactionStatus(entities)
    else:
        result = statusDesc
    return ("Txn no : %s <br> Customer Name : %s <br> status :<b>%s</b> <br>") % (entities['txnNo'], responseDict.get(
        'customerDetail').get('firstName') + " " + responseDict.get('customerDetail').get('lastName'), result)
