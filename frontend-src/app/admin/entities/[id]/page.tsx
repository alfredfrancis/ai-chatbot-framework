...
const EntityPage = ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  const addValue = () => {
    setFormData(prev => ({
      ...prev,
      values: [...prev.values, { value: '', synonyms: [] }]
    }));
  };
  ...
  const handleSynonymInputKeyDown = (index: number, e: KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && synonymInputs[index]?.trim()) {
      ...
      if (!formData.values[index].synonyms.includes(newSynonym)) {
        updateValue(index, 'synonyms', [...formData.values[index].synonyms, newSynonym]);
      }
      setSynonymInputs(prev => ({
        ...prev,
        [index]: ''
      }));
    } else if (e.key === 'Backspace' && !synonymInputs[index] && formData.values[index].synonyms.length > 0) {
      // Remove the last synonym when backspace is pressed on empty input
      const newSynonyms = [...formData.values[index].synonyms];
      newSynonyms.pop();
  ...
  const removeSynonym = (valueIndex: number, synonymIndex: number) => {
    const newSynonyms = [...formData.values[valueIndex].synonyms];
